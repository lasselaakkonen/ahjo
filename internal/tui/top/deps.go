...
type Deps struct {
	ResolveContainerName func(*registry.Worktree) (string, error)
	FormatExposed        func([]ports.Allocation) string
	HostStatus           func() HostStatus

	// ToggleExpose flips the worktree between "all listening ports
	...
	ToggleExpose func(*registry.Worktree) (string, error)
}

// HostStatus is the right-pane content shown when no repo/worktree is selected.
